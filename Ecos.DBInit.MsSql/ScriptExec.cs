...
namespace Ecos.DBInit.MsSql
{
	...
	{
		private static SqlConnection _executionConnection;
		...
		public virtual void TryConnectionAndExecuteInsideTransaction (Script script)
		{
			...
			foreach (var strCommand in strCommands.Where(cmd => !String.IsNullOrEmpty(cmd.Trim()))) {
				var command = new SqlCommand (strCommand,GetExecutionConnection());
		...
		//TODO: Refactor repited code
		private static SqlConnection GetExecutionConnection ()
		{
		...
		{
			_executionConnection = new SqlConnection (_connectionString);
